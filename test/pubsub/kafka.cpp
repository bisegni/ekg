...
TEST(KafkaPB, KafkaSimplePubSub)
{
    SubscriberInterfaceElementVector messages;
    std::unique_ptr<IPublisher> producer = std::make_unique<RDKafkaPublisher>();
    std::unique_ptr<ISubscriber> consumer = std::make_unique<RDKafkaSubscriber>();
...
TEST(KafkaPB, KafkaPushMultipleMessage)
{
