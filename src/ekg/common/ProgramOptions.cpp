...
ProgramOptions::ProgramOptions() {
    options.add_options()("help", "Show all the options")
        ...
        (CMD_MAX_FECTH_CMD, po::value<unsigned int>(), "The max number of command fetched per consume operation")
        (CMD_MAX_FETCH_TIME_OUT, po::value<unsigned int>(), "Specify the timeout for waith the command in microseconds")
        ...
        (SUB_GROUP_ID, po::value<std::string>(), "Subscriber group id")
...
ConstLogConfigurationUPtr ProgramOptions::getloggerConfiguration() {
    return std::make_unique<const LogConfiguration>(
        LogConfiguration{.log_on_console = GET_OPTION(LOG_ON_CONSOLE, bool, false),
